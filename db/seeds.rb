# ##artists
n = 1
Artist.destroy_all
10.times do 
   Artist.create(name: "Artist #{n}", abstract: Faker::ChuckNorris.fact, 
    bio: Faker::Hacker.say_something_smart, password: "flatiron", 
    username: "artist#{n}@.com", artist_type: "Photographer",
    img_link: Faker::Avatar.image("image#{n}", "200x200"))
   n += 1
end

##fans
Fan.destroy_all
n = 1
20.times do 
  fan =Fan.create(name: "fan #{n}", bio: Faker::ChuckNorris.fact, 
    password: "flatiron", username: "fan#{n}@.com",
    img_link: Faker::Avatar.image("image#{n}", "200x200"))
    n += 1
  fan.cart=Cart.create()
  fan.save
end

##image collections
word_arr = %w[Tampflex Opela Wrapsafe Kanlam Greenlam Lotlux Cardguard Treeflex Temp Job Domainer Tampflex Redhold Solarbreeze Viva Voyatouch Matsoft Cardify Sub-Ex Lotstring]

ImageCollection.destroy_all
n = 0
20.times do 
  ImageCollection.create(name: word_arr[n], artist_id: rand(1..10),
    img_link: Faker::Avatar.image("image#{n}", "200x200"))
  n += 1
end

##media 
genre_arr = ["Horror", "Humor", "Biography/Autobiography", "Comic/Graphic", "Novel", "Western", "Science", "Fiction"]

Medium.destroy_all
i = 1
n = 0
40.times do 
  Medium.create(name: "medium #{i}",  
    quantity: rand(1..100), genre: genre_arr[n], image_collection_id: rand(1..20),
    img_link: Faker::Avatar.image("image#{i}", "200x200"))
    n += 1
    i += 1
    n = 0 if n >= genre_arr.length
end

##@items
Item.destroy_all
100.times do
  Item.create(cart_id: rand(1..20), medium_id: rand(1..40))
end


 